...
RSA 因 Rivest、Shamir、Adleman 三人姓氏得名，非对称里最常见的一档：TLS、签名、登录协议里都能见到。下面先记公钥私钥各干什么，再往下挖数学和实现里容易翻车的点。

> **别自己拼填充**：加密用 **OAEP**、签名用 **PSS**（或库默认），手撸 PKCS#1 v1.5 加密等于给 Bleichenbacher 送人头。
...
专利 2000 年到期之后，库和协议里 RSA 才彻底「随便用」；时间线当故事看即可。
...
和考试一样：**先接受结论再算例题**；真要工程实现请直接调 **BigInt / bignum**，别在 JS 里手写 pow。
...
教科书五步：**选 p、q → n、φ(n) → 选 e → 求 d**。生产上 **p、q 要密码学安全随机**，不是 `rand()`。
...
**1024 别上新系统**；**2048** 仍是常见底线；更长密钥 **算得更慢**，按合规文档（如 NIST）和证书年限选。
...
| RSA-640 | 640 bits（约 193 位十进制数） | 2005 年 | F. Bahr 等 |
| RSA-768 | 768 bits（约 232 位十进制数） | 2009 年 | 国际团队 |
| RSA-250 | 829 bits | 2020 年 | Boudot 等 |

## RSA 的两种主要用途

工程里 **很少用 RSA 直接加密大块数据**——多是 **握手里包一层对称密钥** 或 **验证书链**；下面示意图是原理。
...
**PKCS** 把「数学上的 RSA」变成 **文件里能存的字节**：私钥是 PKCS#1 还是 PKCS#8、证书是 X.509，对接 OpenSSL / 语言库时经常会撞这些名字。
...
**教科书 RSA**（裸 `m^e mod n`）Deterministic、可乘、可被开方——上线必加 **OAEP / PSS**。
...
TLS 1.3 里 **密钥交换多是 ECDHE**，RSA 更多出现在 **证书签名** 和老握手；别死记「HTTPS = RSA 加密一切」。
...
**ECC 用更短密钥换类似安全强度**；老系统、某些 HSM 仍只认 RSA——选型看 **互通和设备**，别唯参数论。
...
演示 **库怎么用**；密钥生成、随机数、侧信道，一律信 **成熟实现 + 审计过的模块**。
...
**RSA 只搬小块**（对称密钥、短 token）；**AES/ChaCha** 扛流量——这是 TLS、PGP、代码里最常见的分工。
...
- **安全假设**：大整数分解难；密钥短了、实现漏了，数学再漂亮也白搭。  
- **长度**：新系统 **≥2048**；长期根 CA 常见 **4096**，看政策。  
- **填充**：加密 **OAEP**，签名 **PSS**；v1.5 加密能避就避。  
- **工程**：私钥进 **HSM/KMS**，公钥指数 **65537**，大数据走 **混合加密**。  
- **路线**：新协议多推 **ECC**；**后量子**  hybrid 在路上，RSA 还会苟很多年。
