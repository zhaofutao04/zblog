...
3DS 1.0 从 Visa 2001 年那版算起，算老前辈：**弹窗、整页跳转** 那套今天看都很硬。
...
2016 年 EMVCo 的 2.0 把重点改成 **Risk-Based Authentication**：能静默就静默，高风险再弹 challenge——**Frictionless** 这个词就是这么来的。
...
1. **2.0 能少弹窗**：多喂 **设备/交易字段**，Frictionless 比例才上得去；字段缺一堆，ACS 只能老实地 challenge。  
2. **责任转移要读细则**：不是「接了 3DS 就万事大吉」，看网络、MCC、金额档。  
3. **欧洲 PSD2 / SCA**：3DS 2.0 是常见落地壳，具体仍要对照收单行清单。  
4. **小商户别自己造轮子**：MPI / 3DS Server 扔给 **PSP** 托管，省一堆认证和联调。

> 3DS 只是拼图一块：**PCI、Token、P2PE** 该谁扛还谁扛，别指望一个协议包打天下。
