...
Day 1 说过：LLM 是一个带参数 \(\theta\) 的函数 \(f_\theta\)，训练时用损失推着 \(\theta\) 动。今天要回答：**\(\theta\) 长什么样、前向怎么算、损失是什么、梯度下降在干什么** —— 仍用最小数学，但会完成一次**纸笔手推**。
...
其中 \(\sigma'(z) = \sigma(z)(1-\sigma(z))\)。这就是 Day 3 **反向传播**要系统化推广到「任意计算图」的同一种规则。
