...
4. 用 **均方误差（MSE）** 这类标量损失，对 **玩具网络里两个权重 \(w_1,w_2\)** 手算 \(\partial L/\partial w\)（链式法则热身；含偏置时共 4 个可学习量，见 §7）。  
...
| **GELU** | （略复杂，有近似式） | GPT-2/3 等 FFN 常用；**LLaMA/Qwen 等更新模型多用 SwiGLU**，Day 10 再对齐。 |
...
常见是 **GELU** 或 **SwiGLU**（视模型代际）；Day 10 会对着结构再记一版。
